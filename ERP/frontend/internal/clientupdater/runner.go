...
const Version = "cbmp-client-updater/1.0.0"
...
func (r *Runner) Run(ctx context.Context) error {
	...
	for {
		if err := r.RunOnce(ctx); err != nil {
			r.logger.Printf("client updater run failed: %v", err)
		}
...
func (r *Runner) handleTask(ctx context.Context, task ProductSystemUpdateTask) error {
	r.logger.Printf("handling client update task %s component=%s action=%s target=%s", task.TaskNo, task.Component, task.Action, task.Version)
	_ = r.client.Report(ctx, task.TaskNo, ReportRequest{Status: "running", Progress: 10, Step: "accepted", Message: "客户端更新器已接收任务"})
	var releaseDir string
	var err error
	switch strings.ToLower(strings.TrimSpace(task.Action)) {
	case "rollback":
		_ = r.client.Report(ctx, task.TaskNo, ReportRequest{Status: "running", Progress: 35, Step: "rollback_prepare", Message: "客户端更新器正在准备回滚"})
		releaseDir, err = r.installer.Rollback(ctx, task)
	default:
		_ = r.client.Report(ctx, task.TaskNo, ReportRequest{Status: "running", Progress: 25, Step: "download", Message: "客户端更新器正在下载更新包"})
		download, artifact, downloadErr := r.client.DownloadTask(ctx, task)
		if downloadErr != nil {
			err = downloadErr
			break
		}
		_ = r.client.Report(ctx, task.TaskNo, ReportRequest{Status: "running", Progress: 55, Step: "install", Message: "客户端更新器正在写入蓝绿发布目录"})
	...
	if err != nil {
		if r.cfg.AutoRollbackOnFailure && !strings.EqualFold(task.Action, "rollback") {
			_ = r.client.Report(ctx, task.TaskNo, ReportRequest{Status: "running", Progress: 85, Step: "auto_rollback", Message: "客户端更新器执行失败，正在自动回滚上一版本", Error: err.Error()})
			rollbackTask := task
			...
			if rollbackErr == nil {
				...
				message := "客户端更新器更新失败，已自动回滚上一版本，发布目录 " + rollbackDir
		...
		reportErr := r.client.Report(ctx, task.TaskNo, ReportRequest{Status: "failed", Progress: 100, Step: "failed", Message: "客户端更新器执行失败", Error: err.Error()})
		if reportErr != nil {
