...
func NewService(cfg Config, sink Sink, logger *log.Logger) *Service {
	return &Service{cfg: cfg, sink: sink, logger: logger, deduper: NewDeduper(cfg.DedupeWindow)}
}
...
func (s *Service) healthz(w http.ResponseWriter, _ *http.Request) {
	writeJSON(w, http.StatusOK, map[string]string{"status": "ok", "service": "gps-forwarder"})
}
...
func (s *Service) metricsJSON(w http.ResponseWriter, _ *http.Request) {
	writeJSON(w, http.StatusOK, map[string]uint64{
		...
		"duplicates":    s.metrics.Duplicates.Load(),
...
func (s *Service) ingestHTTP(w http.ResponseWriter, r *http.Request) {
	...
	loc, duplicate, err := s.Ingest(r.Context(), raw, "http", protocol, source)
	if err != nil {
	...
	writeJSON(w, status, map[string]any{"duplicate": duplicate, "location": loc.withoutRaw()})
}

func (s *Service) Ingest(ctx context.Context, raw, channel, protocol, source string) (Location, bool, error) {
	s.metrics.Received.Add(1)
	loc, err := ParseLocation(raw, source, channel, protocol, time.Now())
	if err != nil {
		s.metrics.Rejected.Add(1)
		return Location{}, false, err
	}
	if s.deduper.Seen(loc, time.Now()) {
		s.metrics.Duplicates.Add(1)
		return loc, true, nil
	}
	s.metrics.Accepted.Add(1)
	if err := s.sink.Write(ctx, loc); err != nil {
...
func (s *Service) handleConn(ctx context.Context, conn net.Conn) {
	...
	for scanner.Scan() {
		...
		if _, duplicate, err := s.Ingest(ctx, line, "tcp", s.cfg.DefaultProtocol, s.cfg.DefaultSource); err != nil {
			_, _ = fmt.Fprintf(conn, "ERR %s\n", err.Error())
		} else if duplicate {
			_, _ = io.WriteString(conn, "DUP\n")
		} else {
...
func (s *Service) readNewLines(ctx context.Context, path string, offset int64) (int64, error) {
	...
	for scanner.Scan() {
		line := strings.TrimSpace(scanner.Text())
		if line != "" {
			if _, _, err := s.Ingest(ctx, line, "file", s.cfg.DefaultProtocol, s.cfg.DefaultSource); err != nil {
